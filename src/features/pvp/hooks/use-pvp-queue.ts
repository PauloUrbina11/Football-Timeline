...
/**
 * Cola de matchmaking simple (ver plan de arquitectura del módulo PvP): entra a la cola, escucha
 * en tiempo real la fila propia de `pvp_queue` (sin polling) hasta que `status` pase a "matched",
 * con un respaldo por si el cambio ocurrió en la ventana antes de que la suscripción quedara
 * activa (`getActivePvpMatch`, la misma consulta que ya sirve para reconexión).
 */
export function usePvpQueue() {
  ...
  return { status, matchId, errorMessage, search, cancel };
}
