...
select v.title, v.description, v.event_date::date, 'day', v.display_date,
       (select id from subjects where slug = 'copa-mundial-catar'), '{}'::jsonb
from (
  values
    ('Arabia Saudita vence a Argentina', 'Una de las mayores sorpresas de la fase de grupos de la historia de los mundiales.', '2022-11-22', null),
    ('Marruecos elimina a Portugal', 'Marruecos se convierte en el primer equipo africano en llegar a semifinales de un Mundial.', '2022-12-10', null),
    ('Francia elimina a Inglaterra', 'Francia se impone en un cruce de cuartos de final muy disputado.', '2022-12-10', null),
    ('Argentina vence a Francia en la final', 'Argentina se corona campeona del mundo tras una final decidida en la tanda de penaltis.', '2022-12-18', null)
...
select 'tournament', (select id from subjects where slug = 'copa-mundial-catar'), 'easy',
       ...
       'Ordena cronológicamente estos acontecimientos del torneo.', 'draft', true
